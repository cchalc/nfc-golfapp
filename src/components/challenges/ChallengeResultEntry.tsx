import { Avatar, Button, Flex, RadioGroup, Text } from "@radix-ui/themes";
import { Trophy } from "lucide-react";
import type { Challenge, ChallengeResult, Golfer } from "../../db/collections";
import {
...
export function ChallengeResultEntry({
	...
	function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
		...
		const deletePromises = existingResults.map((existing) =>
			deleteChallengeResult.mutateAsync({
				id: existing.id,
	...
	const isPending =
		createChallengeResult.isPending || deleteChallengeResult.isPending;
	...
	return (
		<form onSubmit={handleSubmit} data-testid="result-entry-form">
			<Flex direction="column" gap="4">
				...
				<RadioGroup.Root
					name="winner"
					defaultValue={existingWinner?.golferId || ""}
				>
