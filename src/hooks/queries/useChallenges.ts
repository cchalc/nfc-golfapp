...
import {
	getChallengeResults,
	getChallengeResultsByTripId,
	getChallenges,
	getChallengesByTripId,
} from "../../server/queries";
...
export const challengeKeys = {
	...
	byTrip: (tripId: string) =>
		[...challengeKeys.lists(), "trip", tripId] as const,
};
...
export const challengeResultKeys = {
	...
	byTrip: (tripId: string) =>
		[...challengeResultKeys.lists(), "trip", tripId] as const,
};
	...
	return useMutation({
		...
		onSuccess: (_, challenge) => {
			...
			queryClient.invalidateQueries({
				queryKey: challengeKeys.byTrip(challenge.tripId),
			});
			queryClient.invalidateQueries({
				queryKey: tripKeys.detail(challenge.tripId),
			});
		},
	});
}
...
export function useCreateChallengeResult() {
	const queryClient = useQueryClient();
	return useMutation({
		mutationFn: ({ result }: { result: ChallengeResult; tripId?: string }) =>
...
export function useUpdateChallengeResult() {
	...
	return useMutation({
		mutationFn: ({
			...
			id: string;
			changes: Partial<
				Omit<ChallengeResult, "id" | "challengeId" | "golferId">
			>;
			tripId?: string;
