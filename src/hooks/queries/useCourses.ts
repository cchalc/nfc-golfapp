...
export const teeBoxKeys = {
	...
	byCourse: (courseId: string) =>
		[...teeBoxKeys.all, "course", courseId] as const,
};
...
export const holeKeys = {
	...
	byCourse: (courseId: string) =>
		[...holeKeys.all, "course", courseId] as const,
};
...
export function useUpdateCourse() {
	...
	return useMutation({
		mutationFn: ({
			...
			id: string;
			changes: Partial<Omit<Course, "id">>;
	...
	return useMutation({
		...
		onSuccess: (_, teeBox) => {
			queryClient.invalidateQueries({
				queryKey: teeBoxKeys.byCourse(teeBox.courseId),
			});
		},
	});
}
...
export function useUpdateTeeBox() {
	...
	return useMutation({
		mutationFn: ({
			...
			id: string;
			changes: Partial<Omit<TeeBox, "id">>;
			courseId: string;
...
export function useCreateHole() {
	...
	return useMutation({
		...
		onSuccess: (_, hole) => {
			queryClient.invalidateQueries({
				queryKey: holeKeys.byCourse(hole.courseId),
			});
...
export function useImportCourseWithDetails() {
	...
	return useMutation({
		...
		onSuccess: (_, { course }) => {
			...
			queryClient.invalidateQueries({
				queryKey: teeBoxKeys.byCourse(course.id),
			});
