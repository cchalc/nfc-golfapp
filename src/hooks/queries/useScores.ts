...
import { deleteScore, insertScore, updateScore } from "../../server/mutations";
...
export const scoreKeys = {
	...
	byRound: (roundId: string) =>
		[...scoreKeys.lists(), "round", roundId] as const,
};
	...
	return useMutation({
		...
		onSuccess: (_, score) => {
			...
			queryClient.invalidateQueries({
				queryKey: scoreKeys.byRound(score.roundId),
			});
			queryClient.invalidateQueries({
				queryKey: roundSummaryKeys.byRound(score.roundId),
			});
		},
	});
}
