...
export const tripGolferKeys = {
	...
	byTrip: (tripId: string) =>
		[...tripGolferKeys.lists(), "trip", tripId] as const,
...
export function useCreateTripGolfer() {
	...
	return useMutation({
		mutationFn: (tripGolfer: TripGolfer) =>
		...
		onSuccess: (_, tripGolfer) => {
			...
			queryClient.invalidateQueries({
				queryKey: tripKeys.detail(tripGolfer.tripId),
			});
		},
	});
}
...
export function useUpdateTripGolfer() {
	...
	return useMutation({
		mutationFn: ({
			...
			id: string;
			changes: Partial<Omit<TripGolfer, "id">>;
