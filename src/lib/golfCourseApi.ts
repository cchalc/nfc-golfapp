...
console.log(
	"Golf Course API Key configured:",
	API_KEY ? "Yes (length: " + API_KEY.length + ")" : "NO - check .env file",
);
...
export async function searchCourses(query: string): Promise<ApiCourse[]> {
	if (!API_KEY) {
		console.error(
			"Golf Course API key not configured. Set VITE_GOLF_COURSE_API_KEY in .env file.",
		);
 ...
 */
export function getAllTees(
	course: ApiCourse,
): Array<ApiTee & { gender: "male" | "female" }> {
