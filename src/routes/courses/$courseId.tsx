...
import {
	getCourse as fetchCourseFromApi,
	getAllTees,
	getPrimaryTee,
...
function CourseDetailPage() {
	...
	const roundsUsingCourse =
		allRounds?.filter((r) => r.courseId === courseId) ?? [];
	...
	function handleGenerateHoles() {
		...
		const strokeIndices = [
			7, 15, 3, 11, 1, 13, 5, 9, 17, 8, 16, 4, 12, 2, 14, 6, 10, 18,
		];
	...
	async function handleResyncCourse() {
		...
		try {
			...
			// Build new holes
			const teesWithHoles = allTees.filter(
				(t) => t.holes && t.holes.length > 0,
			);
			const teeForHoles =
				teesWithHoles.find((t) => t.gender === "male") || teesWithHoles[0];
			...
			const result = await resyncCourseDetails({
				data: {
					courseId,
					courseUpdates,
					teeBoxes: newTeeBoxes,
					holes: newHoles,
				},
			});

			console.log(
				`Resynced course with ${result.teeBoxCount} tee boxes and ${result.holeCount} holes`,
			);
		} catch (error) {
							...
							<AlertDialog.Root>
								<AlertDialog.Trigger>
									<Button
										...
										title={
											isInUse
												? "Course is used by rounds and cannot be deleted"
												: "Delete course"
										}
									>
										<Trash2 size={14} />
								...
								<AlertDialog.Content maxWidth="450px">
									...
									<AlertDialog.Description size="2">
										Are you sure you want to delete "{course.name}"? This will
										also delete all hole and tee box data. This action cannot be
										undone.
					...
					<Card>
						<Flex align="center" gap="2" py="1">
							...
							<Text size="2" color="gray">
								This course is used by {roundsUsingCourse.length} round(s) and
								cannot be deleted.
							</Text>
						</Flex>
					</Card>
												...
												<Table.Cell>
													<Flex align="center" gap="2">
														...
														<Badge
															size="1"
															variant="soft"
															color={tee.gender === "male" ? "blue" : "pink"}
														>
															{tee.gender === "male" ? "M" : "W"}
														</Badge>
													</Flex>
				...
				<Flex direction="column" gap="3">
					<Flex justify="between" align="center">
						...
						<Flex gap="2">
							...
							<Dialog.Root
								open={addHoleDialogOpen}
								onOpenChange={setAddHoleDialogOpen}
							>
								<Dialog.Trigger>
									<Button size="1">
										<Plus size={14} />
