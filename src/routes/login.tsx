...
function LoginPage() {
	...
	const isExactLoginPath =
		loginMatch !== undefined && !window.location.pathname.includes("/verify");
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		try {
			const result = await requestMagicLink({ data: { email: trimmedEmail } });
			if (!result.success) {
				setError(result.error || "Unable to send login code.");
				return;
			}
