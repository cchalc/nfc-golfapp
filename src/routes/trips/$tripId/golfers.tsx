...
import { Check, ChevronLeft, Edit2, X } from "lucide-react";
...
function TripGolfersPage() {
	...
	const tripGolferMap = new Map(
		(tripGolfers || []).map((tg) => [tg.golferId, tg]),
	);
	const tripGolferIds = new Set((tripGolfers || []).map((tg) => tg.golferId));

	// Filter golfers for this trip
	const sortedGolfers = [...(globalGolfers || [])].sort((a, b) =>
		a.name.localeCompare(b.name),
	);
	const addedGolfers = sortedGolfers.filter((g) => tripGolferIds.has(g.id));
	const availableGolfers = sortedGolfers.filter(
		(g) => !tripGolferIds.has(g.id),
	);
	...
	function toggleGolfer(golferId: string) {
		...
		if (existing) {
			...
			if (duplicateCheck) {
				console.warn(
					`Golfer ${golferId} already in trip ${tripId}, skipping duplicate`,
				);
				return;
			}
	...
	function startEditingHandicap(
		tripGolfer: TripGolfer,
															...
															{editingHandicap === tripGolfer?.id ? (
																<Flex align="center" gap="1">
																	<TextField.Root
																		...
																		value={handicapValue}
																		onChange={(e) =>
																			setHandicapValue(e.target.value)
																		}
																		style={{ width: 60 }}
																		min={0}
																		max={54}
