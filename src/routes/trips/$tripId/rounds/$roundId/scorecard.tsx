...
export const Route = createFileRoute(
	"/trips/$tripId/rounds/$roundId/scorecard",
)({
	ssr: false,
...
function ScorecardPage() {
	...
	const golfer = useMemo(() => golferMap.get(golferId), [golferMap, golferId]);
	...
	const { prevGolfer, nextGolfer } = useMemo(() => {
		...
		return {
			...
			nextGolfer:
				index < playingGolfers.length - 1 ? playingGolfers[index + 1] : null,
		};
	}, [playingGolfers, golferId]);

	const navigateToGolfer = useCallback(
		(newGolferId: string) => {
			navigate({
					...
					<Flex justify="between" align="center">
						...
						<Flex direction="column" align="center" gap="1">
							...
							<Text size="1" color="gray">
								HCP{" "}
								{(
									tripGolferMap.get(golferId)?.handicapOverride ??
									golfer?.handicap
								)?.toFixed(1)}
