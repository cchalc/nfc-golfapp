...
async function requireScoreAccess(
	roundId: string,
	golferId: string,
): Promise<void> {
...
async function requireScoreAccessById(scoreId: string): Promise<void> {
  ...
  `;
	...
	await requireScoreAccess(
		scores[0].round_id as string,
		scores[0].golfer_id as string,
	);
}
...
export const updateScore = createServerFn({ method: "POST" })
	.inputValidator(
		(data: {
			id: string;
			changes: Partial<Omit<Score, "id" | "roundId" | "golferId" | "holeId">>;
		}) => data,
	)
